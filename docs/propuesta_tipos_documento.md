# Propuesta: Tipos de Documento para Estimados (Annual / Particular)
...
| Opción 2 | **Particular** | Propuestas específicas |
...
En ambos casos, el usuario definirá el tipo de documento (**Annual** o **Particular**) al momento de generarlo, y el sistema producirá la versión correspondiente.
...
3. El sistema identifica automáticamente:
   ...
   - El tipo de documento (Annual o Particular).
4. El sistema genera el archivo Word correspondiente y lo deja disponible en Drive con su enlace, igual que hoy.

Para garantizar la continuidad operativa, si no se especifica un tipo de documento, el sistema aplicará el valor por defecto (**Annual**). Esto evita interrupciones en los procesos existentes.
...
|-------|-----------------|-------------------|-------------|
| `document_type` | `annual`, `particular` | `annual` | No |
...
| Evento total | Particular | Estimado (Particular) |
| Por día | Annual | Estimado por día (Annual) |
| Por día | Particular | Estimado por día (Particular) |

El sistema valida el tipo recibido y, ante un valor no reconocido, aplica el valor por defecto para no detener la generación.
...
- Retrocompatibilidad: solicitudes sin el campo `document_type` siguen generando el documento por defecto.
