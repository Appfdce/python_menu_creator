...
payload = {
    ...
    "extras_events": [
        # Day 1 - rentals
        {"date_header": "October, Tuesday 27 2026", "rental": True, "name": "Tent", "price": "1000"},
        {"date_header": "October, Tuesday 27 2026", "rental": True, "name": "Chairs", "price": "200"},
        # Day 1 - sales
        {"date_header": "October, Tuesday 27 2026", "rental": False, "name": "Flowers", "price": "50"},
        # Day 1 - provided by client (excluded from totals, shown as such)
        {"date_header": "October, Tuesday 27 2026", "rental": True, "name": "Stage", "price": "999", "provide_by_client": True},
        # Day 2 - only sales
        {"date_header": "October, Wednesday 28 2026", "rental": False, "name": "Linens", "price": "75"},
    ],
    "financials": {
        "tax_rate": "0",
        "tax_name": "Tax",
...
def test_extras_grouped_by_date_and_type(generator_cls):
    ...
    assert "October, Tuesday 27 2026" in text
    assert "October, Wednesday 28 2026" in text

    # Each extra line shows name + individual price
    assert "Tent\t$ 1,000.00" in text
    assert "Chairs\t$ 200.00" in text
    assert "Flowers\t$ 50.00" in text
...
def test_extras_financial_totals_from_price(generator_cls):
    ...
    # Rentals = Tent(1000) + Chairs(200) = 1200 (Stage excluded)
    assert "Extras Services (Rentals)" in text
    assert "$ 1,200.00" in text

    # Sales = Flowers(50) + Linens(75) = 125
